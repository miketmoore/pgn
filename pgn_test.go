...
var parsed = pgn.PGN{
	...
	Movetext: pgn.Movetext{
		pgn.MovetextEntry{White: pgn.PlayerMove{File: pgn.FileE, Rank: pgn.Rank4}, Black: pgn.PlayerMove{File: pgn.FileE, Rank: pgn.Rank5}},
...
func TestParse(t *testing.T) {
	got, err := pgn.Parse(raw)
	if err != nil {
		t.Fatal(fmt.Sprintf("Parsing test failed: %s", err))
	}
