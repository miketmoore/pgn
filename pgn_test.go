		...
		Game{
			...
			Movetext: Movetext{
				...
				MovetextEntry{
					...
					Black: Move{Original: "hxg5", File: FileG, Rank: Rank5, Piece: PiecePawn, Capture: true, Disambiguate: Disambiguate{File: FileH}},
				},
				MovetextEntry{
				...
				MovetextEntry{
					White: Move{Original: "axb4", File: FileB, Rank: Rank4, Piece: PiecePawn, Capture: true, Disambiguate: Disambiguate{File: FileA}},
					Black: Move{Original: "cxb4", File: FileB, Rank: Rank4, Piece: PiecePawn, Capture: true, Disambiguate: Disambiguate{File: FileC}},
				},
				MovetextEntry{
...
func TestUnmarshal(t *testing.T) {
	...
	if len(got.Games) != 1 {
		t.Fatal("total games is unexpected: ", len(got.Games))
	}
	game := got.Games[0]
	if game.TagPairs != unmarshalled.Games[0].TagPairs {
		fmt.Printf("Got:\n%v\n", game)
		fmt.Printf("Expected:\n%v\n", unmarshalled)
		t.Fatal("Parsing PGN string failed")
	}
	if !assertMovetextEquality(unmarshalled.Games[0].Movetext, game.Movetext) {
		fmt.Printf("Got:\n%v\n", game)
		fmt.Printf("Expected:\n%v\n", unmarshalled)
